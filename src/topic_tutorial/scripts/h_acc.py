...
from std_msgs.msg import Float64, Float32 #added
import numpy as np
...
class localPathPlanning :
    ...
    def __init__(self):
        ...
        rospy.Subscriber('/TLStopDist', Float32, self.tl_stop_callback) #added
        ...
        self.vel_planning=velocityPlanning(20,0.3)
        ...
        while not rospy.is_shutdown():
            if self.is_path and self.is_pose and self.is_obj==True and self.is_current_vel==True :
                ...
                target_velocity = cc.acc(local_vaild_object,self.current_vel,self.vel_profile[current_waypoint], self.tl_stop_dist) 
    ...
    #added
    def tl_stop_callback(self,msg):
        self.tl_stop_dist = msg.data
    ...
    def calc_vaild_obj(self,obj_msg,ego_pose):
        ...
        if self.all_object.num_of_npcs > 0:
            ...
            for num in range(self.all_object.num_of_npcs):
                ...
                if local_result[0][0]>0 :
                    global_object_info.append([1,self.all_object.npc_list[num].position.x,self.all_object.npc_list[num].position.y,self.all_object.npc_list[num].velocity.x/3.6])
                    loal_object_info.append([1,local_result[0][0],local_result[1][0],self.all_object.npc_list[num].velocity.x/3.6])
        ...
        if self.all_object.num_of_pedestrian > 0:
            ...
            for num in range(self.all_object.num_of_pedestrian):
                ...
                local_result=tmp_det_t.dot(global_result)
                if local_result[0][0]>0 :
                    global_object_info.append([0,self.all_object.pedestrian_list[num].position.x,self.all_object.pedestrian_list[num].position.y,self.all_object.pedestrian_list[num].velocity.x/3.6])
                    loal_object_info.append([0,local_result[0][0],local_result[1][0],self.all_object.pedestrian_list[num].velocity.x/3.6])
...
class cruiseControl: ## ACC(advanced cruise control) 적용 ##
    ...
    def acc(self,local_vaild_object,ego_vel,target_vel,tl_stop): ## advanced cruise control 를 이용한 속도 계획 ##
        out_vel=target_vel
        pre_out_vel = out_vel
