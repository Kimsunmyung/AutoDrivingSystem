...
class gilbot_pubsub:
    ...
    def cal_gps_heading(self, new_utm_x, new_utm_y):
        ...
        heading_pi = self.old_heading_pi
        heading_degrees = self.old_heading_degrees
        delta_x = new_utm_x - self.old_utm_x
        delta_y = new_utm_y - self.old_utm_y
        
        if abs(delta_x) < distance_limit and abs(delta_y) < distance_limit: return heading_rad, heading_pi, heading_degrees
        if abs(delta_x) < self.gps_status_msg.position_error and abs(delta_y) < self.gps_status_msg.position_error: return heading_rad, heading_pi, heading_degrees
        if delta_x == 0.0 and delta_y == 0.0: return heading_rad, heading_pi, heading_degrees
    ...
    def gps_publish(self):
        try:
            ...
            rad, pi, degree = self.cal_gps_heading(self.gps_status_msg.utm_x, self.gps_status_msg.utm_y)
            self.gps_status_msg.heading_rad = rad
            self.gps_status_msg.heading_pi = pi
...
if __name__=='__main__':
    ...
    except: start_port = "ttyACM0" #default serial port
